...
close all
%% load dataset
X_train = readmatrix("cleveland_X_train.csv");
Y_train_original = readmatrix("cleveland_Y_train.csv");
Y_train = ((Y_train_original > 0.5));
...
for p = 1:length(sigma_n)
    ...
    for trial = 1:Trial
        ...
        for iter = 1:J
            ...
            for m = 1:num_parties
                w_t_temp_ss(:,:,m) = w_t_ss(:,:,m) - lr_dist/(num_parties*n_col)*XTgXwy_ss(:,:,m);
            end
